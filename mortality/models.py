...
class UserProfile(models.Model):
    ...
    drinking_habits = models.CharField(max_length=10, choices=DRINKING_CHOICES, default='none')
    exercise_frequency = models.CharField(max_length=10, choices=EXERCISE_CHOICES, default='none')
    bmi_category = models.CharField(max_length=15, choices=BMI_CHOICES, default='normal')
    sleep_quality = models.CharField(max_length=10, choices=SLEEP_CHOICES, default='good')
    
    # Family history
    family_longevity = models.IntegerField(default=75, help_text="Average age of death of parents/grandparents")
    
    # Lifestyle
    stress_level = models.IntegerField(default=5, help_text="Stress level 1-10 (10 being extremely stressed)")
    social_connections = models.BooleanField(default=True, help_text="Do you have strong social connections?")
    
    # Social Network Features
    anonymous_username = models.CharField(max_length=50, unique=True, blank=True, null=True)
    allow_social_sharing = models.BooleanField(default=False)
    mortality_score = models.IntegerField(default=0)  # Points for productive activities
    ...
    def generate_anonymous_username(self):
        """Generate anonymous username for social features"""
        if not self.anonymous_username:
            import random
            import string
            adjectives = ['Mortal', 'Finite', 'Fleeting', 'Brief', 'Temporal', 'Ephemeral', 'Transient', 'Passing']
            nouns = ['Soul', 'Being', 'Spirit', 'Wanderer', 'Seeker', 'Dreamer', 'Thinker', 'Observer']
            
            adjective = random.choice(adjectives)
            noun = random.choice(nouns)
            number = random.randint(100, 999)
            
            self.anonymous_username = f"{adjective}{noun}{number}"
            self.save()
...
class MortalityReflection(models.Model):
    ...
    is_public = models.BooleanField(default=False)  # For social sharing
    created_at = models.DateTimeField(auto_now_add=True)
    
    def __str__(self):
        return f"{self.user.username}'s reflection - {self.created_at.strftime('%Y-%m-%d')}"

# NEW SOCIAL NETWORK MODELS

class MortalityChallenge(models.Model):
    CHALLENGE_TYPES = [
        ('time_waste', 'Reduce Time Waste'),
...
class ChallengeParticipant(models.Model):
    challenge = models.ForeignKey(MortalityChallenge, on_delete=models.CASCADE)
    user = models.ForeignKey(User, on_delete=models.CASCADE)
...
class MortalityPost(models.Model):
    POST_TYPES = [
        ('reflection', 'Mortality Reflection'),
        ('achievement', 'Life Achievement'),
...
class MortalityComment(models.Model):
    post = models.ForeignKey(MortalityPost, on_delete=models.CASCADE, related_name='comments')
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    content = models.TextField()
    is_anonymous = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)
...
class MortalityLeaderboard(models.Model):
    LEADERBOARD_TYPES = [
        ('least_waste', 'Least Time Wasted'),
        ('most_productive', 'Most Productive'),
        ('best_habits', 'Best Habits'),
